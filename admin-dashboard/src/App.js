...
const AdminDashboard = () => {
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("មានបញ្ហាការតភ្ជាប់!", { id: loadingToast }); 
    }
  };

  // --- ៥. ការគ្រប់គ្រងស្ថានភាព Order & Stock ---
  ...
  const handleDelete = async (type, id) => {
    if (!id) return toast.error("រកមិនឃើញ ID!");
      ...
      <main className="flex-1 p-6 md:p-10 overflow-y-auto max-h-screen">
        ...
        {activeTab === 'dashboard' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in duration-500">
            ...
            <StatCard label="Pending" count={data.orders.filter(o => o.status === 'Pending').length} icon={<ShoppingCart size={28}/>} color="amber" />
            <StatCard label="Banners" count={data.banners.length} icon={<ImageIcon size={28}/>} color="purple" />
            <StatCard label="Revenue" count={`$${data.orders.filter(o => o.status === 'Completed').reduce((sum, o) => sum + Number(o.total || 0), 0).toLocaleString()}`} icon={<BarChart3 size={28}/>} color="emerald" />
          </div>
        )}
        ...
        {activeTab === 'products' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              <h3 className="text-2xl font-black uppercase italic text-slate-800">Inventory</h3>
              <button 
                ...
                className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-black flex items-center gap-2 shadow-lg hover:bg-blue-700 active:scale-95 transition-all"
              >
            ...
            <div className="relative mb-4">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-white border border-slate-100 rounded-2xl outline-none font-bold shadow-sm focus:ring-2 focus:ring-blue-500"
        ...
        {activeTab === 'orders' && (
          <div className="space-y-6">
            <h3 className="text-2xl font-black uppercase italic text-slate-800">Orders</h3>
            <OrderTable 
